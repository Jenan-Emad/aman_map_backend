...
hazardSchema.statics.pointInGazaPolygon = function (lat, lng) {
  let inside = false;
  for (let i = 0, j = GAZA_POLYGON.length - 1; i < GAZA_POLYGON.length; j = i++) {
    const xi = GAZA_POLYGON[i][0], yi = GAZA_POLYGON[i][1];
    const xj = GAZA_POLYGON[j][0], yj = GAZA_POLYGON[j][1];
